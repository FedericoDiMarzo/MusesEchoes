...
"""Create all synths in config file--look in the current directory, in the directory above the braidaa module, and in /usr/local/braidaa"""
synths = {}
try:
    ...
    pass
try:
    with open(os.path.join(os.path.dirname(__file__), "../braidaa", "synths.yaml")) as f:
        synths.update(yaml.safe_load(f))
except FileNotFoundError as e:
    pass
try:
    with open("/usr/local/braidaa/synths.yaml") as f:
        synths.update(yaml.safe_load(f))
except FileNotFoundError as e:
    pass
if len(synths):
